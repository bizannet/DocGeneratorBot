"""
Вспомогательные обработчики и утилиты.

Этот файл содержит общие утилиты и вспомогательные функции для обработки:
...
Основные функции:
- format_user_info: форматирует информацию о пользователе
- safe_edit_message: безопасно редактирует сообщение с обработкой ошибок
- create_main_menu: создает основное меню
- handle_unknown_callback: обрабатывает неизвестные callback-запросы
- format_order_summary: форматирует информацию о заказе
- send_long_message: отправляет длинные сообщения с разбивкой
- format_datetime: форматирует дату и время
- is_valid_user: проверяет валидность пользователя
"""
